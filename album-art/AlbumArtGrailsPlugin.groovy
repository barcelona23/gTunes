class AlbumArtGrailsPlugin {
    ...
    def title = "Album Art look-up plugin" // Headline display name of the plugin
    def author = "Adrian Rodriguez"
    def authorEmail = "me@adriancr.com"
    def description = '''\
A plug-in that provides facilities to look-up album art
'''

    // URL to the plugin's documentation
    def documentation = "http://grails.org/plugin/album-art"
    ...
    def doWithDynamicMethods = { ctx ->
        ...
        if (albumClass){
            albumClass.metaClass.getArt = {->
                albumArtService.getAlbumArt(delegate.artist?.name, delegate.title)
            }
        }
    }
